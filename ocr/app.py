...
def solve_latex_ocr_question():
    """
    DEPRECATED: This endpoint has been moved to the proxy service.
    
    The LaTeX OCR integration functionality has been moved from the OCR service
    to the Proxy service for better performance and architecture.
    
    Please use the proxy endpoint instead:
    POST /api/latex-ocr-solve (on proxy service)
    
    Reason for deprecation:
    - Decomposition is now processed directly in proxy without further OCR scanning
    - Reduces latency by eliminating double OCR service calls
    - Simplifies error handling and debugging
    """
    return jsonify({
        'error': 'This endpoint has been deprecated and moved to the proxy service',
        'message': 'Please use POST /api/latex-ocr-solve on the proxy service instead',
        'reason': 'Architectural change: Processing now happens directly in proxy',
        'migration_date': '2025-11-18',
        'documentation': 'See ARCHITECTURE_UPDATE.md for details'
